...
transfer_file() {
    ...
    if scp $SCP_OPTIONS "$local_file" "$SSH_HOST:$remote_path" 2>&1; then
        ...
        remote_size=$($SSH_CMD "du -h '$remote_path' 2>/dev/null | cut -f1" || echo "FAILED")
...
$SSH_CMD "touch data/raw/.gitkeep"
